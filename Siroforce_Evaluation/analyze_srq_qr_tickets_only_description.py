...
def load_workbook_with_lock_fallback(excel_file: Path) -> tuple[object, Path | None]:
  try:
    return openpyxl.load_workbook(excel_file, read_only=True, data_only=True), None
  except PermissionError:
    with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
      tmp_path = Path(tmp.name)
    shutil.copy2(excel_file, tmp_path)
    return openpyxl.load_workbook(tmp_path, read_only=True, data_only=True), tmp_path
...
def load_rows_from_web_archive(excel_file: Path) -> list[tuple[object, ...]]:
  msg = BytesParser(policy=policy.default).parsebytes(excel_file.read_bytes())
  sheet_part = None
  for part in msg.walk():
    if part.get_content_type() == "text/html":
      content_location = part.get("Content-Location", "")
      if content_location.endswith("sheet001.htm"):
        sheet_part = part
        break

  if sheet_part is None:
    raise ValueError("Could not find embedded sheet HTML in web archive workbook")

  parser = HtmlTableRowParser()
  parser.feed(sheet_part.get_content())
  return parser.rows


def load_sheet_values(excel_file: Path, sheet_name: str) -> tuple[list[tuple[object, ...]], Path | None]:
  if excel_file.suffix.lower() == ".xls":
    try:
      ...
      values = [tuple(sheet.row_values(row_idx)) for row_idx in range(sheet.nrows)]
      return values, None
    except Exception:
      return load_rows_from_web_archive(excel_file), None

  workbook, temp_copy_path = load_workbook_with_lock_fallback(excel_file)
  try:
    worksheet = workbook[sheet_name]
    values = [tuple(row) for row in worksheet.iter_rows(values_only=True)]
    return values, temp_copy_path
  finally:
    workbook.close()
...
def load_rows_from_sheet(excel_file: Path, sheet_name: str) -> list[dict[str, object]]:
  rows_values, temp_copy_path = load_sheet_values(excel_file, sheet_name)
  try:
    header_row_idx, headers = find_header_row_from_values(rows_values)
    ...
    if missing:
      ...
      raise ValueError(f"Required columns are missing: {missing_txt}")

    rows: list[dict[str, object]] = []
    for row in rows_values[header_row_idx:]:
      if not row or all(cell is None for cell in row):
        continue

      transaction = normalize_text(row[col_index["Transaction Number"]])
      if not transaction:
        continue

      hub = normalize_text(row[col_index["Support Hub (old)"]])
      description = normalize_text(row[col_index["Description"]]) if "Description" in col_index else ""
      cat4_val = normalize_text(row[col_index["Category Level 4"]]) if "Category Level 4" in col_index else ""
      desc_primary, desc_secondary = classify_description(description, cat4=cat4_val)
      row_obj = {
        "Transaction Number": transaction,
        "CreatedAt": normalize_created_at(row[col_index["Created at (Day)"]]),
        "Record Type": normalize_text(row[col_index["Record Type"]]),
        "Category Level 2": normalize_text(row[col_index["Category Level 2"]]),
        "Category Level 3": normalize_category_level_3(row[col_index["Category Level 3"]]),
        "Category Level 4": normalize_category_level_4(row[col_index["Category Level 4"]]),
        "Firmware Version": normalize_text(row[col_index["Firmware Version"]]),
        "Description": description,
        "Description Primary": desc_primary,
        "Description Secondary": desc_secondary,
        "Support Hub (old)": hub,
        "Tickets": normalize_ticket_count(row[col_index["Tickets"]]),
        "Region": classify_region(hub),
        "RecordTypeGroup": classify_record_type_group(normalize_text(row[col_index["Record Type"]])),
      }
      rows.append(row_obj)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Auswertung fuer SRQ_QR_TICKETS_Z1 erstellen und als HTML speichern.",
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--sheet",
        default="IO 24Month",
        help="Name des auszuwertenden Worksheets.",
    )
    parser.add_argument(
        "--output",
      default="Ticket_Report_only_Description.html",
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"Excel Datei nicht gefunden: {input_path}")

    rows = load_rows_from_sheet(input_path, args.sheet)
    ...
    print(f"Report erstellt: {output_path.resolve()}")
    print(f"Datensaetze verarbeitet: {report_data['summaries']['ALL']['rows']}")
